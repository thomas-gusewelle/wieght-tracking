...
const Weight = ({
  ...
  const handlePreSubmit = (e) => {
    ...
    if (
      ...
      setTargetWeightError({
        state: true,
        message: "Please Enter Your Target Weight",
      });
    } else {
  ...
  useEffect(() => {
    if (
      !isValidating &&
      !currentWeightError.state &&
      !targetWeightError.state
    ) {
      handleSubmit();
    }
          ...
          <form className='flex flex-col'>
            ...
            <input
              className={`py-2 px-4 rounded-md focus:outline-none focus:ring-2 ${
                currentWeightError.state && "border-red-500 border-2"
              }`}
              type='number'
              id='currentWeight'
              value={currentWeight}
            ...
            <span className='text-red-500'>
              {currentWeightError.state && currentWeightError.message}
            </span>
            ...
            <input
              className={`py-2 px-4 rounded-md focus:outline-none focus:ring-2 ${
                targetWeightError.state && "border-red-500 border-2"
              }`}
              type='number'
              id='targetWeight'
              value={targetWeight}
            ...
            />
            <span className='text-red-500'>
              {targetWeightError.state && targetWeightError.message}
            </span>
