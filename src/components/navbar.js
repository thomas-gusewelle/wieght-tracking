    ...
    <div className='fixed w-screen h-16 bg-stone-900'>
      <div className='h-full relative px-6 flex items-center md:justify-between'>
        ...
        <div
          ...
          } absolute top-16 right-0 h-screen w-2/3 flex flex-col items-center px-6 gap-4 pt-8 bg-stone-900 text-white transition-all duration-75 ease-in-out md:flex md:items-center md:gap-6 md:static md:flex-row md:h-auto md:w-auto md:pt-0 md:translate-x-0`}>
          <Link href='/dashboard'>
            <a
              onClick={() => setShowMobileNav(false)}
        ...
        <button
          ...
          className='absolute right-4 top-1/2 -translate-y-1/2 text-white hover:text-green-500 md:hidden'>
          {showMobileNav ? (
