...
const NameSignup = ({ signUpState, signUpStateUp, firstName, setFirstName, lastName, setLastName}) => {

    const [firstNameError, setFirstNameError] = useState({state: false, message: ""});
    ...
    const handleNext = (e) => {
       ...
       // setIsValidating(true);
        ...
        if (firstName.trim() === ""){
            setFirstNameError({ state: true, message: "Please Enter Your First Name" });
        ...
        <div className="max-w-lg w-full">
          ...
          <div className="flex flex-col">
            ...
            <label htmlFor="firstName" className="text-gray-200">
              ...
              <input
                className={`py-2 px-4 rounded-md focus:outline-none focus:ring-2 ${firstNameError.state && 'border-red-500 border-2'}`}
                type="text"
                id="firstName"
                value={firstName}
                onChange={(e) => setFirstName(e.target.value)}
              />
              <span className="text-red-500">{firstNameError.state && firstNameError.message}</span>

              <label htmlFor="lastName" className="mt-6 text-gray-200">
                Last Name
              </label>
              <input
                className={`py-2 px-4 rounded-md focus:outline-none focus:ring-2 ${lastNameError.state && 'border-red-500 border-2'}`}
                type="text"
                id="lastName"
                value={lastName}
                onChange={(e) => setLastName(e.target.value)}
              />
              <span className="text-red-500">{lastNameError.state && lastNameError.message}</span>
